<template>
  <div class="relationship-type-schemas">
    <!-- Search and Filter -->
    <v-row class="mb-4">
      <v-col cols="12" md="6">
        <v-text-field
    ...
    <v-dialog v-model="showTemplates" max-width="800px">
      <v-card>
        <v-card-title class="d-flex justify-space-between align-center">
          <span>Relationship Schema Templates</span>
    ...
    <v-dialog v-model="showDeleteDialog" max-width="400px">
      <v-card>
        ...
        <v-card-text>
          ...
          the relationship schema "{{ schemaToDelete?.name }}"?
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
const loadSchemas = async () => {
  ...
  try {
    ...
    const response = await schemaStore.fetchRelationshipTypeSchemas(params)
    schemas.value = response.schemas
    totalSchemas.value = response.total_count
  } catch (error) {
    console.error('Failed to load relationship schemas:', error)
  } finally {
    loading.value = false
  }
}
...
const loadTemplates = async () => {
  try {
    templates.value = await schemaStore.fetchRelationshipTypeTemplates()
  } catch (error) {
    console.error('Failed to load relationship templates:', error)
  }
}
...
const viewSchema = (schema) => {
  router.push(`/schemas/relationship-types/${schema.id}`)
}

const editSchema = (schema) => {
  router.push(`/schemas/relationship-types/${schema.id}/edit`)
}
...
const confirmStatusToggle = async () => {
  try {
    await schemaStore.updateRelationshipTypeSchema({
      id: schemaToDelete.value.id,
      is_active: !schemaToDelete.value.is_active
    })
    loadSchemas()
  } catch (error) {
    console.error('Failed to update relationship schema status:', error)
  } finally {
...
const createFromTemplate = async (template) => {
  try {
    // For relationship schemas, we need to create them manually since templates might not be available
    const newSchema = {
      name: template.name,
    ...
    const createdSchema = await schemaStore.createRelationshipTypeSchema(newSchema)
    showTemplates.value = false
    router.push(`/schemas/relationship-types/${createdSchema.id}/edit`)
  } catch (error) {
